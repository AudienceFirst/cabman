            ...
            foreach($data as $block) {
                ...
                if($block->name === 'title') {
                    ...
                    if($block->image === 'featured') {
                        ...
                        $post->thumbnail_data = wpb_getImageBySize(array('attach_id' => (int)$block->image, 'thumb_size' => $grid_thumb_size, 'class' => 'img-rounded'));
                    } else {
...
$li_span_class = 'col-md-' . floor( 12 / $grid_columns_count );
...
<div class="<?php echo apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $css_class, $this->settings['base']) ?>">
    <div class="wpb_wrapper">
        ...
        <div class="teaser_grid_container">
            ...
            <!-- Isotope filter -->
            <div class="isotope-filter-content img-rounded">
                <i class="arrow_triangle-down"></i>

                <ul class="categories_filter clearfix nav nav-pills">
                    <li class="active col-md-2"><a href="#" data-filter="*"><?php _e('All', 'js_composer') ?></a></li>
                    <?php foreach($this->getFilterCategories() as $cat): ?>
                    <li class="col-md-2"><a href="#" data-filter=".grid-cat-<?php echo $cat->term_id ?>"><?php echo esc_attr($cat->name) ?></a></li>
                    <?php endforeach; ?>
                </ul>
            </div>
            ...
            <?php endif; ?>
            <ul class="row wpb_thumbnails wpb_thumbnails-fluid clearfix" data-layout-mode="<?php echo $grid_layout_mode ?>">
