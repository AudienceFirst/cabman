...
function st_bbp_list_forums( $args = '' ) {
		...
		global $post;
		$output = $sub_forums = $topic_count = $reply_count = $counts = $forum_icon_html = '';
		$i = 0;
		$count = array();
		...
		if ( !empty( $sub_forums ) ) {
			...
			foreach ( $sub_forums as $sub_forum ) {
				...
				// Get forum icon
				$forum_icon = get_post_custom_values( 'gg_forum_icon',$sub_forum->ID );
				if ($forum_icon != '') {
	    		    $forum_icon_html = '<i class="'.$forum_icon[0].' pull-left gg-forum-icon"></i>';
	    		} else {
	    			$forum_icon_html = '';
	    		}
				...
				$output .= "<li class='{$class}'><ul>" . $link_before . '<div class="bbp-forum-title-container media">'.$forum_icon_html.'<div class="media-body"><a href="' . $permalink . '" class="bbp-forum-link">' . $title . '</a><p class="bbp-forum-description">' . $description . '</p></div></div>' . $counts . $freshness_link . $link_after . "</ul></li>";
			}
